...
class LightningModule(
    ...
    def to_torchscript(
        ...
        if method == 'script':
            ...
            example_inputs = self.transfer_batch_to_device(example_inputs, self.device)
            torchscript_module = torch.jit.trace(func=self.eval(), example_inputs=example_inputs, **kwargs)
        else:
            raise ValueError("The 'method' parameter only supports 'script' or 'trace', but value given was:"
                             f" {method}.")
        self.train(mode)
